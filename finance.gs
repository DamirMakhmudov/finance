...
function getModelFinance(userEmail){
  ...
  let 
    ...
    // sheetArchive = sp.getSheetByName('Менеджер.Архив'),
    // addressValsArchive = sheetArchive.getRange(2, sheetArchive.getRange('1:1').getValues()[0].indexOf('Адрес объекта')+1, sheetArchive.getLastRow(), 1).getValues().map(item => ({"label":item[0], "sheet":"Менеджер.Архив"}));
  // addressVals = addressVals.concat(addressValsArchive);
...
function saveDataGSFinance(data){
  let
    ...
    header = sheet.getRange('4:4').getValues()[0],
    values = [];
